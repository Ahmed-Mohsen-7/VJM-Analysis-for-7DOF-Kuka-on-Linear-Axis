...
Ttool2d = eye(4);
Ttool3d = eye(4);
paramsDelta=zeros(4,1);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
acctemp = 0;
acc = 0;
% two step algorithm
    % calculate distance
    count=0;
 %this while loop runs untill we match ideal parameters with real ones.
while 1 
    %this for loop calcultes the difference between the real and ideal position
    %for the three reference points
for i=1:size(q,1)
    % Transformations using ideal params to each measurement point
    ...
    % The difference between real and ideal measurements for each measurement point
    M1dist(:,:,i) = M1(:,:,i) - M1i(:,:,i);
    M2dist(:,:,i) = M2(:,:,i) - M2i(:,:,i);
    M3dist(:,:,i) = M3(:,:,i) - M3i(:,:,i);
    ...
    % recalculate distance with new Tbase and Ttools (calibrated)
    for i=1:size(q,1)
